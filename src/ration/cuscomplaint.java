...
public class cuscomplaint extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					cuscomplaint frame = new cuscomplaint(user);
	 ...
	 * Create the frame.
	 */
	public cuscomplaint(final String user) {
		...
		btnNewButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					...
					String distributor="dis1";
					
					pat=con.prepareStatement("select dis_no from cus where pass=?");
					pat.setString(1, user);
					ra=pat.executeQuery();
					  if(ra.next()==true) {
//						  JOptionPane.showMessageDialog(null,"Record added successfully...");
						  distributor=ra.getString("dis_no");
					  }
					  
					  pat=con.prepareStatement("insert into complaint(c_id,dis_id,request)values(?,?,?)");
					  pat.setString(1, user);
					  pat.setString(2, distributor);
					  pat.setString(3, req);
		...
		btnNewButton.setBounds(1127, 363, 120, 32);
	...
	{
		try {
			pat=con.prepareStatement("select request,response from complaint where c_id=?");
			pat.setString(1, user);
			ra=pat.executeQuery();
